...
export function CommandMenu({ posts }: { posts: Post[] }) {
  ...
  const { blogLinks } = useMemo(
    () => ({
      blogLinks: posts
        .filter((post) => post.metadata?.category === "blogs")
  ...
  return (
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 flex h-10 items-center justify-between gap-2 border-t bg-zinc-100/30 px-4 text-xs font-medium dark:bg-zinc-800/30">
        <CoderNandanMark className="size-6 text-muted-foreground" aria-hidden />
...
function CommandMenuKbd({ className, ...props }: React.ComponentProps<"kbd">) {
  return (
    <kbd
      className={cn(
        "dark:bg_white/10 pointer-events-none flex h-5 min-w-6 items-center justify-center gap-1 rounded-sm bg-black/5 px-1 font-sans text-[13px] font-normal text-muted-foreground shadow-[inset_0_-1px_2px] shadow-black/10 select-none dark:shadow-white/10 dark:text-shadow-xs [&_svg:not([class*='size-'])]:size-3",
        className
      )}
      {...props}
    />
