    ...
    if (isVisible && contentRef.current && tooltipRef.current) {
      const updateDimensions = () => {
        if (contentRef.current && tooltipRef.current) {
          ...
          setWidth(
            tooltipRef.current.offsetWidth || tooltipRef.current.scrollWidth
          );
        }
      };
    ...
    (mouseX: number, mouseY: number) => {
      ...
      if (spaceRight >= tooltipWidth + padding) {
        ...
        finalX = Math.max(
          ...
          viewportWidth - tooltipWidth - containerRect.left - padding
        );
      }
      ...
      if (spaceBottom >= tooltipHeight + padding) {
        ...
        finalY = Math.max(
          padding - containerRect.top,
          viewportHeight - tooltipHeight - containerRect.top - padding
        );
      }
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="pointer-events-none absolute z-[9999] max-w-[90vw] rounded-xl border border-border shadow-popover"
            style={{
              top: position.y,
              left: position.x,
          ...
          >
            <div
              ref={contentRef}
              className="p-4 text-sm leading-relaxed text-neutral-600 dark:text-neutral-400 [&_img]:relative [&_img]:z-10 [&_img]:mb-0 [&_img]:block [&_img]:h-auto [&_img]:max-w-[300px] [&_img]:bg-transparent [&_span]:!opacity-100 [&_span>img]:!opacity-100 [&>*+*]:mt-2 [&>p]:mb-2 [&>p]:leading-relaxed [&>p]:last:mb-0"
            >
              {content}
            </div>
          </motion.div>
